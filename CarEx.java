...
public class CarEx {
    // public static ~~~ : 실행할 클래스에 필요
    public static void main(String[] args) {

// Car 클래스 타입의 객체 myCar 생성
        Car myCar = new Car();
// 객체 myCar 멤버인 keyTurnOn() ,run() 메소드를 호출
// Car 클래스 외부에서 실행하기 때문에 keyTurnOn(), run() 메소드 호출 시
// 객체명이 추가되어 있음
        myCar.keyTurnOn();
        myCar.run();
...
// 반환값이 없기 때문에 메소드의 연산 결과를 외부에서 사용할 수 없음
//void plus(){ ... }
...
// 반환값이 존재하기 때문에 연산결과를 메소드 외부에서 출력
// 메소드의 연산결과를 외부에서 활용할 수 있음
